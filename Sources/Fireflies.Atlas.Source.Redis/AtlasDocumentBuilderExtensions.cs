...
public static class AtlasDocumentBuilderExtensions {
    ...
    public static AtlasDocumentBuilder<TDocument> RedisHashSingleValueSource<TDocument>(this AtlasDocumentBuilder<TDocument> builder, RedisSource source, int database, string key) where TDocument : new() {
        var properties = TypeHelpers.GetAtlasProperties<TDocument>();
        var keyProperties = properties.Where(x => x.Attributes.Any(a => a.GetType().IsAssignableTo(typeof(AtlasKeyAttribute))));
        switch(keyProperties.Count()) {
            case 0:
        ...
        builder.AddSource(new RedisHashSource<TDocument>(source, new HashDescriptor { Database = database, Key = key, KeyProperty = keyProperties.First().Property, ValueProperty = valueProperties.First().Property}));
